<template>
  ...
  <div style="display: flex; justify-content:space-between">
    ...
    <v-select
      ...
      :items="[0, 1]"
      variant="solo"
      v-model="selection_data[0].lower_bound"
    ...
    style="width:200px;"
      ...
      variant="solo"
      v-model="selection_data[0].upper_bound"
      class="select"
    item-text="name"
    hide-details
    single-line
  ...
  <div style="display: flex; justify-content:space-between; margin-top:10px">
    ...
    <div style="padding-top:25px">{{ selection_data[0].lower_bound }}</div>
    <v-text-field
        v-model="selection_data[0].lower_bound_name"
        color="primary"
        label="Lower bound label (optional)"
        variant="underlined"
      ></v-text-field>
    </div>
        <div style="display: flex; justify-content:space-between; margin-top:-40px">
    <div style="padding-top:25px">{{ selection_data[0].upper_bound }}</div>
      <v-text-field
        v-model="selection_data[0].upper_bound_name"
        color="primary"
        label="Upper bound label (optional)"
        variant="underlined"
      ></v-text-field>
    </div>
      </div>
...
import _ from "lodash";
  ...
  setup(props,context) {
      const selection_data = ref(props.selections)
          watch(() => _.cloneDeep(selection_data), (currentValue, oldValue) => {
            console.log("dzień")
      context.emit('update:selections', currentValue.value)
    })
      return {
          selection_data
        }
