...
async function addForm() {
  ...
  const add_form = () => 
  new Promise(r => { r(localStorage.setItem('form ' + (next_form_id + 1), 
...
<div v-if="is_loaded" align="center">
  <ul class="w-full max-w-md">
    <li v-for="(form, index) in forms.value"
      :key="form.form_id"
      ...
      <v-btn
        @click="removeForm(form.form_id)" icon="mdi-delete"><v-icon color="red"></v-icon>
