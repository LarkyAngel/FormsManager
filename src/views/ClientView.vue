...
async function saveAnswer() {
  ...
  if (error_at.value == -1) {
    ...
    const add_answer = () => 
    new Promise(r => { r(localStorage.setItem('answer ' + form_id.value + ' ' + (next_answer_id + 1), 
...
<template>
  ...
  <div v-for="(question, index) in form.value.questions">
           ...
           <v-expansion-panel-text v-bind:class='{ "error": error_at == index }'>
            <keep-alive>
      <component :is="search(question.type+'2')" 
      v-model:modelValue = "answer.value.data[index]"
